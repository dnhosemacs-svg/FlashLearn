...
export function getFlashcard(req: Request, res: Response, next: NextFunction) {
  try {
...
export function postFlashcard(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!created.ok) return res.status(404).json({ message: created.error })
...
export function patchFlashcard(req: Request, res: Response, next: NextFunction) {
  try {
...
export function removeFlashcard(req: Request, res: Response, next: NextFunction) {
  try {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id
